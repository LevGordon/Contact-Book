...
*/
function renderContact({id, picture, name, email, phone, website, address, company }) {
...
function filterByCity(city) {
  ...
  let filtered = window.contacts.filter((contact) => contact.address.city === city)
  return filtered
}
...
function filterHandler() {
  ...
  select.addEventListener("change", (event) => {
    ...
    if(event.target.value === '0') {
      ...
      render(filterByCity(event.target.value))
    }
  }) 
}
...
function loadCities(contacts) {
 const citiesToLoad = contacts.map(({address}) => address.city);
  const uniqueCities = [...new Set(citiesToLoad)];
  let listOptions = "";
  for(let city of uniqueCities){
    listOptions = listOptions + `<option value="${city}">${city}</option>`
  }
  const selectList = document.querySelector("select");
  selectList.innerHTML = `<option value="0">-- Select a city --</option> ${listOptions}`
...
function deleteButtonHandler() {
  ...
  let id = Number(event.target.parentElement.getAttribute("data-id"));
  let newArr = deleteContact(id);
  render(contacts);
}
}
